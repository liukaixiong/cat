 ...
 */
package com.dianping.cat.report.page.tophour;
...
import com.dianping.cat.report.page.tophour.model.ErrorDomainModel;
...
public class Handler implements PageHandler<Context> {
    ...
    private void buildExceptionDashboard(Model model, Payload payload, long date) {
        ...
        TopHourMetric topMetric = new TopHourMetric(minuteCount, payload.getTopCounts(), m_configManager, excludeDomains);
        ...
        Map<String, Domain> domains = report.getDomains();
        List<ErrorDomainModel> errorDomainModelList = new ArrayList<>();
        for (Entry<String, Domain> domainEntry : domains.entrySet()) {
            ErrorDomainModel errorDomainModel = new ErrorDomainModel();
            errorDomainModel.setDomain(domainEntry.getKey());
            Integer domainCount = 0;
            Map<String, Integer> errorInfo = new HashMap<>();
            for (Entry<Integer, Segment> entry : domainEntry.getValue().getSegments().entrySet()) {
                Map<String, Error> errors = entry.getValue().getErrors();
                for (Entry<String, Error> errorEntry : errors.entrySet()) {
                    int count = errorEntry.getValue().getCount();
                    String errorId = errorEntry.getValue().getId();
                    Integer errorCount = errorInfo.get(errorId);
                    if (errorCount != null) {
                        errorCount = errorCount + count;
                    } else {
                        errorCount = count;
                    }
                    errorInfo.put(errorId, errorCount);
                    domainCount = domainCount + count;
                }
            }
            errorDomainModel.setErrorText(errorInfo);
            errorDomainModel.setCount(domainCount);
            errorDomainModelList.add(errorDomainModel);
        }
        model.setErrorDomainModelList(errorDomainModelList);
    }

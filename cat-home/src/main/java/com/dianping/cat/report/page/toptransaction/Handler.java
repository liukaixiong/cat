...
import com.dianping.cat.core.dal.TopDay;
...
import com.dianping.cat.report.page.toptransaction.transform.TransactionTrendGraphBuilder;
...
public class Handler implements PageHandler<Context> {
    ...
    @Inject
    private TopTransactionReportService topTransactionReportService;
    ...
    private void buildTransactionNameGraph(Model model, TransactionReport report, String type, String name, String ip) {
        ...
        if (transactionName != null) {
            String graph1 = m_builder
                    .build(new GraphPayload.DurationPayload("Duration Distribution", "Duration (ms)", "Count", transactionName));
            String graph2 = m_builder.build(new GraphPayload.HitPayload("Hits Over Time", "Time (min)", "Count", transactionName));
            String graph3 = m_builder.build(
                    new GraphPayload.AverageTimePayload("Average Duration Over Time", "Time (min)", "Average Duration (ms)", transactionName));
            String graph4 = m_builder.build(new GraphPayload.FailurePayload("Failures Over Time", "Time (min)", "Count", transactionName));
    ...
    @InboundActionMeta(name = "toptransaction")
    public void handleInbound(Context ctx) throws ServletException, IOException {
        // display only, no action here
    }

    @Override
    @OutboundActionMeta(name = "toptransaction")
...
//        model.setGroups(m_configManager.queryDomainGroup(payload.getDomain()));
    ...
    private void buildTransactionTypeList(Model model, Payload payload) {
        ...
        List<TransactionInfo> transactionTypeList = new ArrayList<>();
        model.setSort(sortBy);
        model.setQueryName(queryName);
        if (StringUtils.isNotEmpty(type)) {

            // 单独处理历史报表
            if (Action.HISTORY_REPORT.equals(action)) {
                List<TopDay> topDays = topTransactionReportService.queryHourlyReport("1", payload.getHistoryStartDate(), "interfaces");
                for (int i = 0; i < topDays.size(); i++) {
                    TopDay topDay = topDays.get(i);
                    if (StringUtils.isNotEmpty(queryName) && !topDay.getDomain().startsWith(queryName)) {
                        continue;
                    }
                    TransactionType transactionType = JSON.parseObject(topDay.getDataJson(), TransactionType.class);
                    TransactionInfo transactionInfo = new TransactionInfo(topDay.getDomain(), transactionType);
                    transactionTypeList.add(transactionInfo);
                }
            } else {
                ...
                for (String domain : domains) {
                    ...
                    TransactionInfo transactionInfo = new TransactionInfo(domain, transactionType);
                    transactionTypeList.add(transactionInfo);
                }
            }
        }
        if (StringUtils.isNotEmpty(sortBy)) {
            Collections.sort(transactionTypeList, ComparatorUtils.nullLowComparator(new TransactionSort(sortBy)));

        }
        model.setTransactionInfo(transactionTypeList);
    }

    private TransactionType getReportData(Model model, String type, TransactionReport report) {
        if (report != null) {
            model.setReport(report);
            Machine machine = report.getMachines().get(Constants.ALL);
            if (machine != null) {
                TransactionType transactionType = machine.getTypes().get(type);
    ...
    private void normalize(Model model, Payload payload) {
        ...
        model.setPage(ReportPage.TOPTRANSACTION);
        if (payload.getAction() != null) {
            model.setAction(payload.getAction());
        } else {
            model.setAction(Action.TOP_VIEW);
        }
